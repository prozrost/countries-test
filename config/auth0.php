...
// Laravel SDK requires clientId even for API (JWT) validation. Use AUTH0_CLIENT_ID, or the same SPA Client ID as VITE_AUTH0_CLIENT_ID.
$auth0ClientId = trim((string) env('AUTH0_CLIENT_ID', ''));
if ($auth0ClientId === '') {
    $auth0ClientId = trim((string) env('VITE_AUTH0_CLIENT_ID', ''));
}
...
return Configuration::VERSION_2 + [
    ...
    'guards' => [
        'default' => [
            ...
            Configuration::CONFIG_CLIENT_ID => $auth0ClientId !== '' ? $auth0ClientId : Configuration::get(Configuration::CONFIG_CLIENT_ID),
